...
class BaseEngine(object):
    ...
    def infer(self, img):
        ...
        :param scales: The ima
            print("data = ", dage resize scales for each image in this batch. Default: No scale postprocessing applied.
    ...
    def inference(self, img_path, conf=0.5, end2end=False):
        ...
        # print("letterbox time", datetime.datetime.now() - starttime)
        data = self.infer(img)
        # print("inference time", datetime.datetime.now() - starttime)
        if end2end:
            # print("data = ", data)
            num, final_boxes, final_scores, final_cls_inds  = data
            ...
            # print("num_classes =", self.n_classes, self.class_names)
            predictions = np.reshape(data, (1, -1, int(5+self.n_classes)))[0]
            # print("reshape time", datetime.datetime.now() - starttime)
            dets = self.postprocess(predictions,ratio)
            # print("postprocess time", datetime.datetime.now() - starttime)

        if dets is not None:
            final_boxes, final_scores, final_cls_inds = dets[:,
                                                             :4], dets[:, 4], dets[:, 5]
            for i in range(len(final_cls_inds)):
                print(self.class_names[int(final_cls_inds[i])], final_scores[i])
            origin_img = vis(origin_img, final_boxes, final_scores, final_cls_inds,
                             conf=conf, class_names=self.class_names)
        return origin_img

    def ecosort_inference(self, img_path, conf=0.5, end2end=False):
        starttime = datetime.datetime.now()
        origin_img = cv2.imread(img_path)
        # img, ratio = preproc(origin_img, self.imgsz, self.mean, self.std)
        img, ratio, dwdh = letterbox(origin_img, self.imgsz)
        ...
        if dets is not None:
            ...
            # for i in range(len(final_cls_inds)):
            #     print(self.class_names[int(final_cls_inds[i])], final_scores[i])
            output = [(self.class_names[int(final_cls_inds[i])], final_scores[i]) for i in range(len(final_cls_inds))]
            return output
        return []
